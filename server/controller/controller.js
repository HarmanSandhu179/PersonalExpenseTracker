  ...
  model.Transaction.aggregate([
    {
      $lookup: {
        from: "categories",
        let: { typePrefix: { $substr: ["$type", 0, 5] } },
        pipeline: [
          {
            $match: {
              $expr: { $eq: [{ $substr: ["$type", 0, 5] }, "$$typePrefix"] },
            },
          },
        ],
        as: "categories_info",
    ...
    .then((result) => {
      // first call categories_info  then call color to avoid errors because color is in that object
